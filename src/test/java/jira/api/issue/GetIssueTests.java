...
public class GetIssueTests {
	...
	// TODO - try generate format - Hod, its generate from header and couple of
	// things. I prefer to leave it as constant. and take it level up(IssueUtils?)?
	// what do you say?

//	The first part is a header, which specifies the type of token and the signing algorithm used.
//	The second part is the payload, which contains claims about an entity and additional data.
//	The third part is a signature, which is used to verify the authenticity of the token and its contents.
	...
	public static String noPermissionToken = generateRandomUUIDString(30);
	...
	public static void userWithoutPermission() {
		Response response = issueService.getIssue(validIssueId, noPermissionToken);
		Assert.assertTrue(response.code() == 404);
		logger.info("the user does not have permission to view it");
	}

	// return random UUID - A universally unique identifier (UUID) is a 128-bit
