...
public class GetIssueTests {
	...
	public static String issueId = "JTP-1";
	...
	public static String noPermissionToken = "Token";
	...
	public static void getIssue() {
		...
		Assert.assertEquals(issueId, getIssueResponse.getKey());
		logger.info("got issue with id: " + issueId);
	}
	...
	public static void incorrectAuthentication() {
		Response response = issueService.getIssue(issueId, invalidToken);
...
//		apiService.login();
		...
		Response response = issueService.getIssue(issueId, noPermissionToken);
		Assert.assertTrue(response.code() == 404);
